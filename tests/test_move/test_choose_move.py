...
class TestChooseMove(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.patchers = [
            ...
            patch.object(self.bot, "select_move", return_value={"move": "left"}, name="random_choice")
        ]
        self.mocks = {}
    ...
    @patch.object(bot.extract_data, "get_neck", return_value=neck)
    @patch.object(bot.extract_data, "edit_body", return_value=body)
    ...
    @patch.object(bot.extract_data, "get_neck", side_effect=RuntimeError("side_effect"))
    ...
    def test_fallback_check_moves(self, mock_edit_body, mock_get_neck, mock_check_moves):
        ...
        mock_reset_is_move_safe = self.mocks.get("reset_is_move_safe", "unknown")
        mock_log_data = self.mocks.get("log_data", "unknown")
        mock_check_safe_moves = self.mocks.get("check_safe_moves", "unknwon")
        mocks_to_assert = {"mock_reset_is_move_safe": mock_reset_is_move_safe, "mock_log_data": mock_log_data, "mock_check_safe_moves": mock_check_safe_moves}
        for name , mock in mocks_to_assert.items():
            if not isinstance(mock, MagicMock):
                raise TypeError(f"Objekt: {name} ist kein MagicMock")
        ...
        mock_reset_is_move_safe.assert_called_once()
        mock_log_data.assert_called_with("choose_move", {"head": self.head, "neck": self.neck, "body": self.body, "my_length": self.my_length})
        self.assertEqual(mock_log_data.call_count, 3)
        mock_check_safe_moves.assert_not_called()
        mock_edit_body.assert_called_once_with(self.body)
        mock_get_neck.assert_called_once_with(body=self.body, game_state=self.game_state)
        mock_check_moves.assert_called_once_with(self.bot.is_move_safe, head=self.head, game_state=self.game_state, body=self.body, neck=self.neck, my_length=self.my_length)
